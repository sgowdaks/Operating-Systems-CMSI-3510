...
pthread_mutex_t fork_mutex [5] = {PTHREAD_MUTEX_INITIALIZER, PTHREAD_MUTEX_INITIALIZER, 
...
int main (int argc, char **argv)
{
    ...
    for (i = 0; i < 5; i++) {
        thread_no [i] = i;
        if ((r = pthread_create (&tid_philosopher [i], NULL, philosopher, (void *) &thread_no [i])) != 0) {
            fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
        }
    }
...
void *philosopher (void *arg)
{
    ...
    while (1) {
        ...
        bool eat = rand () % 2;	
	if (eat) {
            if ((r = pthread_mutex_lock (&waiter_mutex)) != 0) {
                fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
	    }
	    ...
	    // get second fork
	    fork2 = (my_id == 4) ? 0 : my_id + 1;
            if ((r = pthread_mutex_lock (&fork_mutex [fork2])) != 0) {
                fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
            }
            sprintf (my_str, "Philosphers %d and %d got forks... \n", fork1, fork2);
            print_string (my_str);
            sprintf (my_str, "Philosopher %d  is eating....  \n", my_id);
            print_string (my_str);
	    ...
	    // release second fork
            ...
            sprintf (my_str, "Philosopher %d is finished eating...  \n", my_id);
            print_string (my_str);
	    // release waiter
            if ((r = pthread_mutex_unlock (&waiter_mutex)) != 0) {
                fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
            }
	}
	else { //think
            sprintf (my_str, "Philosopher %d is thinking...  \n", my_id);
            print_string (my_str);
            sleep (rand () % 5 + 1);
            sprintf (my_str, "Philosopher %d is finished thinking... \n", my_id);
            print_string (my_str);
	}
    }
}
